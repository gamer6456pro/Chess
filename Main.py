...
def repetition():
    ...
    for x in Board:
        TempPos=''.join(x)
        Pos+=TempPos
    # -----> add something here to check for enpassant and castle rights cuz those are different even if pos is the same so do something liek
    # pos+='castlew' or pos+='castleb and pos+='passant' 
    if EnpassantInPos:
        Pos+='EnPassant'
    for x in RookMoved:
        Pos+=str(x)
    try:
        #also could optimize this by clearing the dict when a piece is captured but too lazy so have to sacrifice cpu by adding this
        #but it saves memory
...
#for the repetition you have to convert the board to string or somethign before making it a key
#also need to somehow check for pawn moves and captures, prob by storing prevoius board and checkign if a piece is missing or something idk
...
def EnemyMoves():
    ...
    for EnemyObject in EnemyGenerator(Board):
        ...
        # -----> change these to fasle 
        # -----> ALTERNATE SOLUTION reimplement the thing to rmeove same color capture in the domove func (idk why it was commented out)
        # but it shouldn't affect anything i hope 
    return EnemyMoves


#Class for handling drawing, capturing, en passant, and turns
class Game():
                ...
                for j,y in enumerate(x):
                    if y[0]==Turn:
                        ...
                        for z in pieceDict[y[1]]:
                            #copy of board is correct so check the .check thing
                            #why are there negatives sometimes fix ig prob with the pawn subtraction im guessing
...
# -----> also add resign and draw and at the start of the game have text indicating which keys those are but then have the text disappear after the game starts         
...
class Piece(): 
    ...
    def pawns(self,board=Board,captureSameColor=False):
        ...
        if self.color=='w':
            ...
            return PossibleMoves
        #ADD EN PASSANT
        #SOMETHING WITH CHECKING IF THE COLUMN AS A PAWN AT THE STARTING POSITION
    ...
    def setType(self,type):
        self.type=type
                ...
                #CHECK FOR INPUT KEY AND CHECK IF A PAWN IS AT THE END OF THE BOARD
    ...
    def castle(self):
        ...
        for i,x in enumerate(CastleIsLegal):
            ...
            if i%2==0:
                ...
                # alirght im not going to move the king cuz i don't feel like it so if theres an error
                # thats unsolveable its prob this thats the problem 
                for x in [2,3]:
    ...
    def EnPassant(self):
        ...
        #follow the same general pattern for enpassant as castle
    #call cenpassant in pawn moves the add to possible moves list
